<#macro fieldInput attrs>
	<#list attrs as attr>
				  <div class="control-group">
				    ...
				    <div class="controls">
				      <input type="text" id="input${attr.attrName?cap_first}" name="${model.className?uncap_first}.${attr.attrName}" value="${r"$"}{${model.className?uncap_first}.${attr.attrName}}">
				    </div>
				  </div>
	</#list>
</#macro>
   ...
   <div class="container" style="margin-top:30px;">
    	<fieldset>
		    <legend>${model.className} Update    <a id="index-btn" href="#" class="button button-rounded button-flat-action" style="margin-left:300px;margin-bottom:10px;"><i class="icon-home"></i>${model.className?uncap_first} list</a>
		    	...
		    	<form class="form-horizontal" action="${r"${contextPath}"}/front/${model.className?lower_case}_update.action">
